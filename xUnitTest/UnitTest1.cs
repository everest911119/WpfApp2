    ...
    public class JsonFileHandleTests
    {
        ...
        public void LoadItemsFromJson()
        {
            ...
            try
            {
                ...
                var handle = new JsonFileHandle(Options.Create(settings), messageBox);
        ...
        [Fact]
        public void LoadItemsFromJson_MissingFile_ShowsMessageBox()
        {
            var settings = new AppSettings
            {
                ItemFileName = $"Item-missing-{Guid.NewGuid():N}.json",
                CategoryRanges = new[] { 0, 500, 1000, 1500 },
                CategoryNames = new[] { "Small", "Medium", "Large", "Oversize" }
            };
            ...
            var result = handle.LoadItemsFromJson();

            Assert.Empty(result);
            Assert.Equal(1, messageBox.CallCount);
            Assert.Contains("File not found", messageBox.LastMessage);
        }

        [Fact]
        public void LoadItemsFromJson_Negative_ShowsMessageBox()
        {
            var settings = new AppSettings
            {
                ItemFileName = "Item-negative.json",
                CategoryRanges = new[] { 0, 500, 1000, 1500 },
                CategoryNames = new[] { "Small", "Medium", "Large", "Oversize" }
            };
            ...
            try
            {
                ...
                Assert.Empty(res);
                Assert.Equal(1, messageBox.CallCount);
                Assert.Contains("negative", messageBox.LastMessage);
            }
            finally
            {
                if (File.Exists(filePath))
        ...
        private  class TestMessageBoxService : IMessageBoxService
        {
